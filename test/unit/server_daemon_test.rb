...
class ServerDaemonTest < Test::Unit::TestCase
  class TestDaemon < Daemon::Base
    ...
    def self.result_fn
      File.join(APP_ROOT, 'script/test.result')
    end
  ...
  def test_working_directory
    assert_equal File.join(APP_ROOT, 'script'), Daemon::WorkingDirectory
  end
  
  def test_pid_fn
    assert_equal File.join(APP_ROOT, 'script/daemon.base.pid'),
      Daemon::Base.pid_fn
    
    assert_equal File.join(APP_ROOT, 'script/serverdaemontest.testdaemon.pid'),
      TestDaemon.pid_fn
  end
